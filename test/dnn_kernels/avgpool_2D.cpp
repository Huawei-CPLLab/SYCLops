...
int main() {
  ...
  deviceQueue.submit([&](handler &cgh) {
    auto kern = [=]() {
        ...
        for (int j = 0; j < OUT_W; j++) {
          for (int k = 0; k < POOL_H; k++) {
            for (int l = 0; l < POOL_W; l++) {
              ...
              out[OUT_W * i + j] += in[W * idx_H + idx_W];
            }
          }
        }
      }
      for (int i = 0; i < OUT_H; i++) {
        for (int j = 0; j < OUT_W; j++) {
          out[OUT_W * i + j] /= POOL_H * POOL_W;
        }
      }
    };
    cgh.single_task<class avgpool_2D>(kern);
  });
...
// MLIR-NEXT:   %cst = arith.constant 4.900000e+01 : f32
// MLIR-NEXT:   %0 = affine.load %arg1[0] : memref<1xf32, 1>
// MLIR-NEXT:   %1 = affine.for %arg2 = 0 to 7 iter_args(%arg3 = %0) -> (f32) {
// MLIR-NEXT:     %3 = affine.for %arg4 = 0 to 7 iter_args(%arg5 = %arg3) -> (f32) {
// MLIR-NEXT:       %4 = affine.load %arg0[%arg4 + %arg2 * 7] : memref<49xf32, 1>
// MLIR-NEXT:       %5 = arith.addf %arg5, %4 : f32
// MLIR-NEXT:       affine.store %5, %arg1[0] : memref<1xf32, 1>
// MLIR-NEXT:       affine.yield %5 : f32
// MLIR-NEXT:     }
// MLIR-NEXT:     affine.yield %3 : f32
// MLIR-NEXT:   }
// MLIR-NEXT:   %2 = arith.divf %1, %cst : f32
// MLIR-NEXT:   affine.store %2, %arg1[0] : memref<1xf32, 1>
// MLIR-NEXT:   return
// MLIR-NEXT: }
// clang-format on
